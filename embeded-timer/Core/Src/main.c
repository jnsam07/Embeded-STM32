...
// ① 1초 주기 토글
static void TIM3_Start_1s(void)
{
  __HAL_TIM_SET_PRESCALER(&htim3, ONESEC_PSC);
  __HAL_TIM_SET_AUTORELOAD(&htim3, ONESEC_ARR);
  __HAL_TIM_SET_COUNTER(&htim3, 0);
  HAL_TIM_Base_Start_IT(&htim3);
}

// ② 설정 가능 "최대" 간격(약 59.65초) 토글
//    T = (PSC+1)*(ARR+1)/TIM3CLK_HZ  = 65536*65536 / 72e6 ≈ 59.65 s
static void TIM3_Start_Max(void)
{
  __HAL_TIM_SET_PRESCALER(&htim3, MAX_PSC);
...
int main(void)
{
  ...
  // 1초 동작 확인 후, 최대 시간으로 바꿔 보려면 이 줄로 교체:
  // TIM3_Start_Max();
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  {
    tim3_isr_cnt++;
    HAL_GPIO_TogglePin(LED6_GPIO_Port, LED6_Pin); // 동작표시등 토글
  }
}
/* USER CODE END 4 */
