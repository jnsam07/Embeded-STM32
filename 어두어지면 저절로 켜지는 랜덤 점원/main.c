...
volatile int hour = 23, minute = 59;


int garden_light_count = 0;
uint32_t adc_value;
...
void display_garden_light(int cnt)
{
    int check_bit=0, random_bit=0, light_on_num=0;
    while (light_on_num < cnt)
    {
        random_bit = rand() % 8;
        if (!(check_bit & (1 << random_bit)))
        {
            HAL_GPIO_WritePin(led_port[random_bit], led_pin[random_bit], GPIO_PIN_SET);
            check_bit |= 1 << random_bit;
            light_on_num++;
        }
    }
}
  ...
  while (1)
  {
      ...
      if (HAL_ADC_PollForConversion(&hadc1, 10) == HAL_OK)
      {
          ...
          if (adc_value > ADC_10LX)
              garden_light_count = 0;
          else if (adc_value > ADC_1LX)
              garden_light_count = 2;
          else if (adc_value > ADC_0DOT1LX)
              garden_light_count = 4;
          else
              garden_light_count = 8;
      }

      display_garden_light(garden_light_count);
      HAL_Delay(GARDEN_LIGHT_CHECK_PERIOD);
